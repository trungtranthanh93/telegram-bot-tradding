    ...
    const job = new cron.CronJob({
        ...
        onTick: async function () {
            ...
            if (currentTimeSecond === parseInt(timeInfo.resultSecond) || currentTimeSecond === (parseInt(timeInfo.resultSecond) + 1) || currentTimeSecond === 19) { // Update kết quả, Thống kê
                ...
                if (checkRowOneForStatistic(timeInfo.rowFirstValue) && isQuickOrder === NON_QUICK_ORDER) {
...
async function getCronTimeInfo() {
    ...
    const RESULT_SETTING_FIRST_ROW = "result.setting.firstrow";
    const ORDER_SETTING_SECOND_ROW = "order.setting.secondrow";
    ...
    let rowFirstValue = await database.getSettingByKey(RESULT_SETTING_FIRST_ROW);
    let rowSecondValue = await database.getSettingByKey(ORDER_SETTING_SECOND_ROW);
...
function checkRowOneForStatistic(value) {
    var createdMinute = new Date().getMinutes();
    if (createdMinute % 2 === value) {
        return false;
    }
    return true;
}

function checkRowOneForOrder(value) {
    var createdMinute = new Date().getMinutes();
    if (createdMinute % 2 === value) {
        return true;
    }
